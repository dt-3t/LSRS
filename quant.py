...
class VectorQuantizer2(nn.Module):
    ...
    # ===================== FOR LSRS =====================
    def idxBl_to_fhat(self, gt_ms_idx_Bl) -> torch.Tensor:
        B = gt_ms_idx_Bl[0].shape[0]
        C = self.Cvae
        H = W = self.v_patch_nums[-1]
        SN = len(self.v_patch_nums)
        ...
        for si in range(SN):
            h_BChw = F.interpolate(self.embedding(gt_ms_idx_Bl[si]).transpose_(1, 2).view(B, C, pn_next, pn_next), size=(H, W), mode='bicubic')
            f_hat.add_(self.quant_resi[si/(SN-1)](h_BChw))
            fhat_list.append(f_hat.detach().clone())
            if si != SN-1:
                pn_next = self.v_patch_nums[si+1]
    ...
    def _get_best_candidate(self, 
        ...
        """Select candidate based on discriminator scores using Top-K sampling.
        
        Args:
            f_hat: Current feature tensor (B, C, H, W)
            h: New feature tensor to add (B*k, C, H, W)
            label_B: Class labels (B,)
            si: Current scale index
            k: Number of candidates per sample
            discriminator: Discriminator model
            top_k: Number of top candidates to sample from (if ==1, equivalent to argmax)
            temperature: Softmax temperature for sampling distribution
    ...
    def get_next_autoregressive_input_lsrs(self, 
                                        si: int, 
                                        SN: int, 
                                        f_hat: torch.Tensor, 
                                        h_tmp: torch.Tensor, 
                                        ...
                                        label_B=None,
                                        top_k: int = 1,
                                        temperature: float = 1.0) -> Tuple[Optional[torch.Tensor], torch.Tensor]:
        """Autoregressive input generation with Top-K sampling support."""
        b, k, c, p1, p2 = h_tmp.shape
        assert discriminator is not None
        HW = self.v_patch_nums[-1]
        h_tmp = rearrange(h_tmp, 'b k c p1 p2 -> (b k) c p1 p2')

        if si != SN - 1:
            h = self.quant_resi[si / (SN - 1)](F.interpolate(h_tmp, size=(HW, HW), mode='bicubic'))
            f_hat = self._get_best_candidate(f_hat, h, label_B, si, k, discriminator,
                                            top_k=top_k, temperature=temperature)
            next_input = F.interpolate(f_hat, size=(self.v_patch_nums[si+1], self.v_patch_nums[si+1]), mode='area')
            return f_hat, next_input
        else:
            h = self.quant_resi[si / (SN - 1)](h_tmp)
            f_hat = self._get_best_candidate(f_hat, h, label_B, si, k, discriminator,
                                            top_k=top_k, temperature=temperature)
            return f_hat, f_hat
